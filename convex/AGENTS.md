...
- **`super_admin` means operator login, not staff.** `dashboard.adminOverview` leaves those rows out of headcount, the school-wide compliance average and the per-phase averages. An operator account belongs to nobody, has no modules and can earn no CPTD points, so counting one would add a person who does not exist to the directory's headcount and drag compliance towards zero — three tiles nobody could reconcile against the staff list. `smt_admin` is a real person with a job title and still counts. Phase 7's staff directory must apply the same exclusion
