...
export const recordLessonProgress = mutation({
  ...
  returns: v.object({ progressPercent: v.number(), moduleCompleted: v.boolean() }),
  handler: async (ctx, args) => {
    const { userId } = await requireStaff(ctx);
    ...
    await ctx.db.patch("users", userId, { compliancePercent, lastActiveAt: now });

    return { progressPercent, moduleCompleted };
  },
});
