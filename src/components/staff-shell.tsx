...
import { LayoutDashboard, BookOpen, Trophy, User, Shield, Menu, Flame, LogOut } from "lucide-react";
 ...
 * The learner chrome.
 *
 * Still prop-driven and Convex-free, per `src/components/AGENTS.md` — the
 * route calls `useStaffViewer()` and passes the result down. Everything below
 * used to be a literal: `MN` initials, a "7-day streak" pill, and a
 * "Switch to SMT" link shown to every teacher in the school whether or not
 * they could use it. All three are now facts about the signed-in person, and
 * absent rather than invented when there is nobody to describe.
 */
export function StaffShell({
  ...
  /** Omitted while unknown; a zero streak renders nothing rather than "0 days". */
  streakDays?: number;
  /** Only an admin is offered the console. The gate still refuses everyone else. */
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col bg-sidebar text-sidebar-foreground lg:flex">
        ...
        <div className="space-y-1 border-t border-sidebar-border p-4">
          ...
          {onSignOut !== undefined && (
            <button
              onClick={onSignOut}
              className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold uppercase tracking-wider text-sidebar-foreground/60 hover:text-gold"
            >
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border bg-background/80 px-4 backdrop-blur lg:px-8">
          ...
          <div className="flex-1">
            <p className="text-xs uppercase tracking-wider text-muted-foreground">
              {viewer === undefined
                ? "Cliffview Academy · Staff Training Portal"
                : `Cliffview Academy · ${viewer.name}`}
            </p>
          ...
          {/*
            The notification bell was a button with no handler and no count.
            Removed rather than left as furniture that does nothing.
          */}
