...
- `loading-screen.tsx` is the Suspense fallback for every route, wired as the router's `defaultPendingComponent` in `src/router.tsx`. Before it existed there was **no** pending component anywhere, and TanStack Router only wraps a route in `React.Suspense` when one is set (`Match.js`: `ResolvedSuspenseBoundary = ... PendingComponent ? Suspense : SafeFragment`) — so the admin routes, which all read with `useSuspenseQuery`, suspended with no boundary above them and blanked the window to white. It paints `bg-background` on the first frame and fades the mark in after a short delay, so a fast navigation shows no spinner at all rather than flashing one. It shows a rotating arc, never a percentage: the query's duration is unknown, and a bar that creeps to 90% and waits is a claim the app cannot support
