...
function getAssetHref(module: TrainingModule, resource: ModuleAsset) {
  return `/academy/admin/modules/${module.slug}/assets/${resource.id}`;
}
...
export function getAdminModuleLibrary(repo: AcademyRepository) {
  const modules = repo.listTrainingModules();

  return {
    summary: "Manage module copy, lessons, and media placeholders before the backend is wired up.",
    modules: modules.map((module) => ({
      ...
      assetCount: module.resources.length,
      updatedLabel: module.lastUpdatedLabel,
      href: `/academy/admin/modules/${module.slug}`,
    })),
  };
}

export function getAdminModuleDetail(repo: AcademyRepository, slug: string) {
  const module = repo.getTrainingModuleBySlug(slug);
  if (!module) {
    throw new Error(`Module not found for slug "${slug}"`);
  }
  ...
  return {
    ...
    outcome: module.outcome,
    objectives: module.learningObjectives,
    previewPath: `/academy/modules/${module.slug}`,
    stats: [
      { label: "Lessons", value: `${module.sections.length}` },
      { label: "Assets", value: `${module.resources.length}` },
      { label: "Pass Mark", value: `${module.passMark}%` },
      { label: "Updated", value: module.lastUpdatedLabel },
    ],
    featuredMedia: featuredAsset
      ? {
          title: featuredAsset.title,
    ...
    lessons: module.sections.map((section) => ({
      ...
      attachedAssets: getSupportingResources(module, section).length,
      state: section.isCurrent ? "current" : section.isComplete ? "published" : "draft",
      href: `/academy/admin/modules/${module.slug}/lessons/${section.id}`,
    })),
    resources: module.resources.map((resource: ModuleAsset) => ({
      id: resource.id,
      title: resource.title,
      kind: resource.kind,
      meta: resource.meta,
      ...
      href: getAssetHref(module, resource),
    })),
    uploadZones: [
      {
        title: "Video placeholder",
        description: "Swap or upload the lesson video shown to staff inside the module player.",
      },
      {
        title: "Audio placeholder",
        description: "Replace narration tracks or spoken summaries tied to each lesson.",
      },
      {
        title: "Documents and guides",
        description:
          "Attach policy PDFs, checklists, and staff handouts that appear in the learner view.",
      },
    ],
  };
}

export function getAdminAssetDetail(repo: AcademyRepository, slug: string, assetId: string) {
  const module = repo.getTrainingModuleBySlug(slug);
  if (!module) {
    throw new Error(`Module not found for slug "${slug}"`);
  }

  const asset = module.resources.find((resource) => resource.id === assetId);
  if (!asset) {
    throw new Error(`Asset not found for module "${slug}" and asset "${assetId}"`);
  }

  const usageReferences = module.sections
    .filter((section) => (section.documentIds ?? []).includes(asset.id))
    .map((section) => ({
      id: section.id,
      title: section.title,
      kind: section.kind,
      durationLabel: section.durationLabel,
      href: `/academy/admin/modules/${module.slug}/lessons/${section.id}`,
      previewHref: `/academy/modules/${module.slug}/lesson/${section.id}`,
    }));

  return {
    moduleTitle: module.title,
    modulePath: `/academy/admin/modules/${module.slug}`,
    previewPath: `/academy/modules/${module.slug}`,
    ...
    placeholderState:
      asset.status === "published"
        ? "Ready for learner-side display"
        : "Draft placeholder pending final upload",
    usageReferences,
    uploadHints: [
      {
        title: "Primary file placeholder",
        description:
          "Swap the current placeholder file with the final uploaded asset when backend storage is ready.",
      },
      {
        title: "Metadata and captioning",
        description: "Add file notes, accessibility labels, and short learner-facing descriptions.",
      },
      {
        title: "Publishing controls",
        description:
          "Decide whether the asset stays draft-only or becomes visible in the learner portal.",
      },
    ],
  };
}

export function getAdminLessonDetail(repo: AcademyRepository, slug: string, lessonId: string) {
  const module = repo.getTrainingModuleBySlug(slug);
  if (!module) {
    throw new Error(`Module not found for slug "${slug}"`);
  }
  ...
  return {
    moduleTitle: module.title,
    modulePath: `/academy/admin/modules/${module.slug}`,
    previewPath: `/academy/modules/${module.slug}/lesson/${lesson.id}`,
    lessonTitle: lesson.title,
    lessonOrderLabel: `Lesson ${lesson.order} of ${module.sections.length}`,
    kind: lesson.kind,
    durationLabel: lesson.durationLabel,
    summary: lesson.summary,
    mediaTitle: lesson.mediaTitle ?? `${lesson.title} media`,
    mediaDescription: lesson.mediaDescription ?? lesson.summary,
    scenarioTitle: lesson.scenarioTitle ?? lesson.title,
    scenarioBody:
      lesson.scenarioBody ??
      `This lesson is where staff will engage with ${lesson.title.toLowerCase()} content for ${module.title}.`,
    reflectionPrompt:
      lesson.reflectionPrompt ??
      "Add the reflection or discussion prompt that should appear beneath the media block.",
    linkedResources: getSupportingResources(module, lesson).map((resource) => ({
      id: resource.id,
      title: resource.title,
      kind: resource.kind,
      meta: resource.meta,
    ...
    uploadZones: [
      {
        title: "Video placeholder",
        description: "Primary visual media used in the academy-side lesson view.",
      },
      {
        title: "Audio placeholder",
        description: "Optional narration or recap audio attached to this lesson.",
      },
      {
        title: "Supporting docs",
        description: "Policy PDFs, worksheets, and templates shown under the lesson content.",
      },
    ],
  };
}
