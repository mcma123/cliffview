...
- **There are two upload orders, and which one a screen uses is a deliberate choice.**
  - *Row first* — `assets.create`, then `useAssetUploads.upload`, then `assets.attachFile`. Used where the row is the subject and may legitimately exist without a file: the asset editor, the module editor's placeholder zones, and `academy.admin.ai-review.tsx`'s Generate tab. A failure between the legs leaves a titled placeholder with no file; the zone says so and offers Remove, which calls `assets.remove`. Do not paper over that case by deleting the row automatically.
  - *Bytes first* — `useAssetUploads.uploadNew`, then `lessons.addMaterial`. Used by the lesson editor's Materials zone, where the admin's subject is the **file**, not a row. The key comes from `assets.generateUploadUrl`, which takes no arguments, so a caller cannot aim at another asset's blob; `addMaterial` then creates the asset, attaches it and publishes it in one transaction. A failed upload leaves nothing to tidy, which is why this order is right when several files are dropped at once — the other order would strand a placeholder per failure.
  - Both legs of the actual transfer live in `use-asset-upload.ts`. Do not write a third implementation of the PUT.
...
- A *row-first* zone exists only where an asset row exists. The module create screen has none: no asset exists until the module does, so the three zones that used to sit there were unfalsifiable by construction and were deleted rather than faked. A *bytes-first* zone has no such constraint — the lesson editor's Materials zone works on a lesson with nothing attached, which is the normal case when building one
