...
- Hosting: the frontend runs on a **Dokploy** server (project `cliffview LMS`, app `cliffview frontend`) at `https://cliffview.cyphersoftai.com`, behind Traefik on container port 3000, with the `...oosn5m-...sslip.io` host as a fallback route. This is an **SSR app**: the process that must run is `node .output/server/index.mjs`, not a static file server. Nixpacks autodetects the repo as a single-page app and starts Caddy, which has no `index.html` to serve and returns a zero-byte 404 on every route while the build still reports success. What currently defuses that is the env var **`NIXPACKS_START_CMD=node .output/server/index.mjs`** — if it is ever dropped, the 404s come straight back. A `Dockerfile` is committed and is the more robust fix, but it only takes effect once the app's build type is switched from `nixpacks` to `dockerfile` in Dokploy, which has not been done
- `VITE_*` values are inlined by Vite **at build time**, so they belong in Dokploy's **build args**, not only its runtime env. Runtime-only produces a bundle with no backend URL in it — a deploy that looks healthy and fails in the browser. Verify after a deploy by grepping the served bundle for the deployment name, not by reading the env list
