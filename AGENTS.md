...
- Hosting: the frontend runs on a **Dokploy** server (project `cliffview LMS`, app `cliffview frontend`) at `https://cliffview.cyphersoftai.com`, behind Traefik on container port 3000. It is built from the repo's `Dockerfile` and served by **`node .output/server/index.mjs`** — this is an SSR app, not a static bundle. Do not let the build type drift back to Nixpacks autodetection: it identifies the repo as a single-page app, builds a Caddy static image, and every route returns a zero-byte 404 while the build still reports success. `VITE_*` values are inlined by Vite at build time, so they belong in Dokploy's **build args**, not only its runtime env — runtime-only yields a bundle with no backend URL and a deploy that looks healthy
